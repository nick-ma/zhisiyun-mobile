    ...
    function($, _, Backbone, Handlebars, moment) {
        ...
        var CollProjectListViewAll = Backbone.View.extend({
            ...
            render: function() {
                ...
                var self = this;
                self.mode = $("#collproject_view_mode2").val() || 'all_project';
                ...
                $("#collproject-left-panel2").trigger("create"); //渲染侧边栏
                // console.log(cp_type);
                $("#btn-collproject_list-add").attr('href', '#collproject_edit/add');

                $("#collproject-content2").html(self.template(render_data));
                $("#collproject-content2").trigger('create');
                // $("#collproject-left-panel2").trigger('create');
            ...
            bind_event: function() {
                var self = this;
                $("#collproject2")
                    .on('click', '#btn_collproject2_cancel', function(event) {
                        event.preventDefault();
                        window.location.href = '#myteam_detail/'+self.people_id+'/basic';
                    })
                    .on('change', '#collproject_view_mode2', function(event) {
                    ...
                    .on('click', '.open-left-panel', function(event) {
                        event.preventDefault();
                        $("#collproject-left-panel2").panel("open");
                    })
                    .on('swiperight', function(event) { //向右滑动，打开左边的面板
                        event.preventDefault();
                        $("#collproject-left-panel2").panel("open");
                    })
                    .on('click', '#btn-collproject-refresh2', function(event) {
                        event.preventDefault();
                        $.mobile.loading("show");
                        self.collection.fetch().done(function() {
                            $.mobile.loading("hide");
                            $("#collproject-left-panel2").panel("close");
                        });
                    })
                    .on('change', '#collproject-left-panel2 input[name=collproject_state2]', function(event) {
                    ...
                    .on('change', '#collproject-left-panel2 select', function(event) {
                        event.preventDefault();
                        var $this = $(this);
