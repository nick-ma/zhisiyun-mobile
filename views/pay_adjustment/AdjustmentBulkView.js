...
define(["jquery", "underscore", "backbone", "handlebars", "async", "moment"], function($, _, Backbone, Handlebars, async, moment) {
    ...
    var AdjustmentbulkView = Backbone.View.extend({
        ...
        initialize: function() {
            this.adjustment_bulk_view_template = Handlebars.compile($("#adjustment_bulk_view_list").html());
        ...
        render: function() {
            ...
            //     rendered_data = self.adjustment_bulk_view_template(self.model.attributes);
            //     // var adjustment_bulk = self.model.get('adjustment_bulk');
            ...
            $('#adjustment_bulk_view #adjustment_bulk_back').attr('href', self.obj.type == 'T' ? '#todo' : '#adjustment_list')

            $("#adjustment_bulk_view-content").html(rendered_data);
            $("#adjustment_bulk_view-content").trigger('create');
            return self;
        },
        bind_event: function() {
            var self = this
            ...
            $("#adjustment_bulk_view").on('click', '#add_pri', function(event) {
                ...
                event.preventDefault();
                var pps = _.map(self.obj.adjustment_bulk.items, function(pp) {
                    var obj = {
                        people_name: pp.people.people_name,
                        val: pp.value
                    }
                    return obj
                })
                ...
                $("#panel-pay_last_view_list_detail").html(option_rendered_data);
                $("#panel-pay_last_view_list_detail").trigger('create');
                $("#panel-pay_last_view_list_detail").panel('open').trigger("updatelayout");
                ...
                if ($("#adjustment_bulk_view-content #ti_comment").val() == '') {
                    alert('请填写审批意见！');
                    return;
                };
                ...
                $.mobile.loading("show");
                var process_define_id = $("#adjustment_bulk_view-content #process_define_id").val();
                var task_define_id = $("#adjustment_bulk_view-content #task_define_id").val();
                var process_instance_id = $("#adjustment_bulk_view-content #process_instance_id").val();
                var task_process_url = $("#adjustment_bulk_view-content #task_process_url").val();
                var task_instance_id = $("#adjustment_bulk_view-content #task_instance_id").val();
                var adjustment_bulk_position = $("#adjustment_bulk_view-content #adjustment_bulk_position").val();
                ...
                self.model.id = $("#adjustment_bulk_view-content #adjustment_bulk_id").val();
                self.model.save().done(function(data1) {
                    $.post('/admin/wf/trans_confirm_form_4m', {
                        ...
                        trans_name: name,
                        ti_comment: $("#adjustment_bulk_view-content #ti_comment").val(),
