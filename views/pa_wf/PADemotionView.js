...
define(["jquery", "underscore", "backbone", "handlebars", "moment", "../../models/PADemotionModel"],
    function($, _, Backbone, Handlebars, moment, PADemotionModel) {
        var paep_id = null;
        var do_trans = function(trans_data) {
                var post_data = {
                    process_instance_id: $("#pa_demotion_hr_list-content #process_instance_id").val(),
                    task_instance_id: $("#pa_demotion_hr_list-content #task_instance_id").val(),
                    process_define_id: $("#pa_demotion_hr_list-content #process_define_id").val(),
                    next_tdid: $("#pa_demotion_hr_list-content #next_tdid").val(),
                    next_user: $("#pa_demotion_hr_list-content #next_user_id").val() || $("#select_next_user").val(), //'516cf9a1d26ad4fe48000001', //以后从列表中选出
                    trans_name: $("#pa_demotion_hr_list-content #trans_name").val(), // 转移过来的名称
                    comment_msg: $("#pa_demotion_hr_list-content #comment_msg").val(), // 任务批注 
                    attachments: trans_data.attachments || null
                };
                var post_url = $("#pa_demotion_hr_list-content #task_process_url").val();
                post_url = post_url.replace('<TASK_ID>', $("#pa_demotion_hr_list-content #task_instance_id").val());
        ...
        var PADemotionView = Backbone.View.extend({
            // The View Constructor
            initialize: function() {
                this.template = Handlebars.compile($("#psh_pa_demotion_hr_view").html());
            ...
            pre_render: function() {
                var self = this;
                $("#pa_demotion_hr_list-content").html(self.loading_template({
                    info_msg: '数据加载中...请稍候'
                }));
                $("#pa_demotion_hr_list-content").trigger('create');
                return this;
            },
            ...
            render: function() {
                ...
                if (self.view_mode == 'trans') {
                    $("#pa_demotion_hr_list #pa_name").html('数据处理人');

                    $("#pa_demotion_hr_list-content").html(self.trans_template(self.trans_data));
                    if (self.trans_data.next_td.node_type == 'END') {
                        do_trans(self.trans_data);
                    }
                } else {
                    $("#pa_demotion_hr_list-content").html(self.template(render_data));
            ...
            bind_events: function() {
                ...
                $("#pa_demotion_hr_list").on('click', '#btn_save', function(event) { //数据保存接口
                    event.preventDefault();
                    paep_id = paep_id || self.collection.models[0].attributes._id;
                    var data_clone = _.clone(self.collection.models[0].attributes);
                    self.collection.models[0].attributes.par = data_clone.par ? data_clone.par._id : null;
                    ...
                    self.collection.models[0].save(self.collection.models[0].attributes, {
                        success: function(model, response, options) {
                            self.collection.url = '/admin/pa/wf/demotion/bb/' + paep_id;
                    ...
                    event.preventDefault();
                    paep_id = paep_id || self.collection.models[0].attributes._id;
                    self.collection.models[0].attributes.self_evaluation = $("#pa_demotion_hr_list #self_evaluation").val();
                    self.collection.models[0].save(self.collection.models[0].attributes, {
                        success: function(model, response, options) {
                            self.collection.url = '/admin/pa/wf/demotion/bb/' + paep_id;
                            self.collection.fetch().done(function() {
                    ...
                    paep_id = paep_id || self.collection.models[0].attributes._id;
                    self.collection.models[0].attributes.validFrom = $("#pa_demotion_hr_list #leaveDate").val();
                    self.collection.models[0].save(self.collection.models[0].attributes, {
                        success: function(model, response, options) {
                            self.collection.url = '/admin/pa/wf/demotion/bb/' + paep_id;
                            self.collection.fetch().done(function() {
                    ...
                    event.preventDefault();
                    var self_evaluation = $("#pa_demotion_hr_list #self_evaluation").val();
                    paep_id = paep_id || self.collection.models[0].attributes._id;
                    ...
                    event.preventDefault();
                    var $this = $(this);
                    if ($("#pa_demotion_hr_list-content #ti_comment").val() == '') {
                        alert('请填写审批意见！');
                        return;
                    }
                    $(this).attr('disabled', true)
                    $.mobile.loading("show");
                    var process_define_id = $("#pa_demotion_hr_list-content #process_define_id").val();
                    var task_define_id = $("#pa_demotion_hr_list-content #task_define_id").val();
                    var process_instance_id = $("#pa_demotion_hr_list-content #process_instance_id").val();
                    var task_process_url = $("#pa_demotion_hr_list-content #task_process_url").val();
                    var task_instance_id = $("#pa_demotion_hr_list-content #task_instance_id").val();
                    ...
                    if (self.view_mode) {
                        $.post('/admin/wf/trans_confirm_form_4m', {
                            ...
                            trans_name: name,
                            ti_comment: $("#pa_demotion_hr_list-content #ti_comment").val(),
                            task_instance_id: task_instance_id,
                            next_tdid: target_id,
                            direction: direction,
                            attachments: self.data.attachments,
                            people_id: $("#people_id").val() || self.collection.models[0].attributes.people._id,
                            position_id: $("#pa_demotion_hr_list-content #dest_position_superior").val(),
                            roles_type: $("#pa_demotion_hr_list-content #roles_type").val(),
                    ...
                    $.mobile.loading("show");
                    if ($("#pa_demotion_hr_list-content #next_user_name").val() || $("#pa_demotion_hr_list-content #select_next_user").val()) {
